# Source and rebuilding @jgdina/node
...
- `package.json`: the exact `build:source` command and pinned development
  tools;
- `scripts/normalize-source-maps.mjs`: deterministic removal of machine-local
  generated-output paths from the CommonJS source maps; and
...
regenerates `dist/**` from `src/**`. Internal runtime dependencies are the exact same-version `@jgdina/core`, `@jgdina/kernels-js`, and `@jgdina/worker-protocol` packages.
